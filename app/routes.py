...
from unredactor_service import create_tokens_tensor, make_predictions, get_decoded_sent, download_blob
...
def unredactor():
    ...
    tokens, index_list = create_tokens_tensor(text, tokenizer)

    # Make list of predictions for masked words.
    predictions = make_predictions(tokens, index_list, tokenizer, model)

    decoded_sent = get_decoded_sent(text, predictions)

    return {"text": text, "unredacted_text": decoded_sent, "unredacted_words": predictions}
